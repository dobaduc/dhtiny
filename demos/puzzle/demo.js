...
$dh.newClass("MAGICVIDEOPUZZLE", {
	...
	onWindowResize: function() {
        ...
        var gtop =  $dh.pos($dh.el("GameContainer")).top;
		this.left = gleft + this.GRIDPADDING.left;//Math.floor(bs.width/2 - this.VWIDTH/2);
		this.top  = this.BGPADDING.top + this.GRIDPADDING.top + gtop;//Math.floor(bs.height/2 - this.VHEIGHT/2);
	...
	makeFirstScreen: function() { // Welcome screen
		...
		M = new DHMotion(div, DHMotion.AnimEffects.bounceEaseOut, 1);
	...
	loadVideoTo: function(divId, width, height, haveSound) {
	    ...
	    var params = { allowscriptaccess: "always",wmode: "opaque", play: this.isFinished ? "true": "false", volume: "0"};
	    ...
	    swfobject.embedSWF(this.currentVideo+"?playSound=var_"+(haveSound?"play":"stop"), divId, width, height, "8", false, flvars, params, atts, onLoadedVideo);
	},
		...
		if (this.isLoadingVideoForPuzzle) {
			...
			for (var i = 0;i < this.nRow; i++) {
				for (var j = 0; j < this.nCol+1; j++) {
					var item = this.Grid.items[i][j];
					if (!item) continue;
					//this.cloneVideoTo(item.vdcontainer, item.id);
	...
	createBlock: function(i, j, rrow, rcol) {
		...
		//if (!this.blocks[i*this.nRow + j] ) { // Create new
	        ...
	        item.vdmask.style.cssText +=";visibility:hidden;background:red;z-index:9999;cursor:url("+$dh.GC.cursorcur+");";
	...
	finishGame: function(dy) {
		...
		for (var i=0; i < this.nRow; i ++)
			for (var j=0; j < this.nCol+1; j++) {
			    var item = this.Grid.items[i][j];
			    if (!item) continue;
                item.canvas.lastLeft = item.left;
                item.canvas.deltaLeft = Math.floor(dy==0?item.width/2:-item.width/2); // 100 percent
		        M = new DHMotion(item.canvas, DHMotion.AnimEffects.bounceEaseOut, 0.25);
                M.setPropValue = function(val) {
                    var newLeft = this.targetObj.lastLeft + this.targetObj.deltaLeft * val/100;
                    this.targetObj.style.left = Math.floor(newLeft) + "px";
                    this.targetObj.ownerObj.vdmask.style.left = (newLeft + self.Grid.left)+ "px";
                }
                P.addChild(M);
                if (i == this.nRow -1 && j == this.nCol) {
                    M.addEv("onstop", function() {
                        self.showRootVideo();
                    });
                }
		...
		for (var i = 0; i < this.nRow; i ++)
			for (var j = 0; j < this.nCol+1 ; j ++) {
				...
				if (item) {
				    ...
				    $dh.css(item.vdmask, {opac:20, bg: "red", border:"solid 1px blue", visibility:"visible"});
				}
			}		
		$dh.setTime(this,"controlAllVideos",["start"],4000);
	},
	...
	controlAllVideos: function(action) {
		...
		case "start":
			/*for (var i=0; i< this.vref.length; i++)
				try {this.vref[i].StartPlay();} catch(errrr) {}*/			
			for (var i=0; i< this.vref.length; i++)
				document.getElementById(this.vref[i]).Play();
