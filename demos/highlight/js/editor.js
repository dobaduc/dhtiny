...
var Editor = (function() {
    ...
    function Editor(options) {
        ...
        if (!options.textWrapping) {
            ...
            this.container.style.whiteSpace = "nowrap"; // By DUC: fixed text wrapping error
        }
        ...
        if (!options.readOnly) {
            ...
            $dh.addEv(document, "keydown", $dh.delegate(this, "keyDown"));
            $dh.addEv(document, "keypress", $dh.delegate(this, "keyPress"));
            $dh.addEv(document, "keyup", $dh.delegate(this, "keyUp"));
            $dh.addEv(document.body, "paste", $dh.delegate(this, "markCursorDirty"));
            $dh.addEv(document.body, "cut", $dh.delegate(this, "markCursorDirty"));
        }
    }
    ...
    Editor.prototype = {
        ...
        getCode: function() {
            ...
            var accum = [];
            forEach(traverseDOM(this.container.firstChild), $dh.delegate(accum, "push"));
        ...
        keyPress: function(event) {
            ...
            if (event.code == 13 || event.code == 9)
                ...
                this.parent.setTimeout($dh.delegate(this, "indentAtCursor"), 0);
        },
        ...
        keyUp: function(event) {
            ...
            // BY DUC - For autocompletion
            if (event.keyCode == 190) {
                //this.delayScanning();
                this.parent.setTimeout($dh.delegate(this, "autoComplete"), 300);
                //this.autoComplete();
            }
            else if (window.editorMask) {
                window.editorMask.show(false);
            }
        },
        // BY DUC -20090310
        autoComplete: function() {
            var curNode = select.selectionTopNode(this.container, false);
            if (curNode.className.indexOf("comment") > 0) // Cannot do anything inside comment string
                return;
            var b = $dh.bounds(curNode);
            var mask = window.editorMask;
            if (!mask) {

                mask = window.editorMask = $dh.New("div", { style: { cssText: ";background:yellow;overflow:auto;border:solid 1px green"}, pos: [0, 0], size: [120, 90], parentNode: document.body });
                ...
                /*mask.onblur = mask.onmousedown = function() {
                    //window.editorMask.show(false);
                    document.body.removeChild(window.editorMask);
                    window.editorMask = null;
                };*/
                //mask.onmousedown = function() { window.editorMask.show(false); }

                mask.show = function(left, top) {
                    if (left == false) {
                        //window.editorMask.style.left = "-3000px";
                        document.body.removeChild(window.editorMask);
                        window.editorMask = null;
                    }
                    else {
                        $dh.pos(window.editorMask, [left, top]);
                    }
                }
            }

            var text = curNode.currentText != "." ? curNode.currentText : curNode.previousSibling.currentText;
            text = text.replace(/^\s+|\s+$/g, "");
            if (text != ".") {
                if (window[text]) {
                    AttLstCreator.loadAttributeList(window[text], mask); //.innerHTML = text;
                    mask.show(b.left + b.width, b.top + b.height);
                }
            }
        },
        ...
        indentLineAfter: function(start) {
            ...
            if (whiteSpace && !$dh.hasClass(whiteSpace, "whitespace"))
                whiteSpace = null;
            ...
            // BY DUC: check if whiteSpace.currentText exist or not
            var whiteSpaceLength = 0;
            if (whiteSpace && whiteSpace.currentText)
                whiteSpaceLength = whiteSpace.currentText.length;
            else if (whiteSpace && whiteSpace.innerHTML)
                whiteSpaceLength = whiteSpace.innerHTML.length;
            var indentDiff = indent - whiteSpaceLength; // OLD: var indentDiff = indent - whiteSpace ? whiteSpace.currentText.length: 0;
        ...
        scheduleHighlight: function() {
            ...
            this.highlightTimeout = this.parent.setTimeout($dh.delegate(this, "highlightDirty"), this.options.passDelay);
        },
        ...
        highlight: function(from, lines, cleanLines) {
            ...
            function correctPart(token, part) {
                return !part.reduced && part.currentText == token.value && $dh.hasClass(part, token.style);
            }
...
// Object browser for Autocompletion - BY DUC
$dh.newClass("AttributeListCreator", {
    loadAttributeList: function(obj, target) {
        target.innerHTML = "";
        for (var p in obj) {
            if (obj[p] == obj || obj[p] == undefined)
                continue;
            var attDiv = $dh.New("div", { html: p + "&nbsp;&nbsp;&nbsp;>" + this.getType(obj[p]) });
            $dh.addEv(attDiv, "mousedown", function() {
                attDiv.style.backgroundColor = "blue";
                attDiv.style.color = "white";
            });
            target.appendChild(attDiv);
        }
    },

    getType: function(obj) {
        if ($dh.isFunc(obj)) return "Function";
        if ($dh.isArr(obj)) return "Array";
        if ($dh.isObj(obj)) return "Object";
        if ($dh.isNum(obj)) return "Number";
        if ($dh.isStr(obj)) return "String";
        return "Unknown";
    }
});
AttLstCreator = new AttributeListCreator();