...
$dh.newClass("DHDialog", DHAnimItem, {
    ...
    setEnableDrag: function(flag) {
        ...
        if (!this.canvas.dragParams) {
            $dh.dragger.setDrag(this.canvas, {
                mode: "RESIZE,MOVE", 
                minwidth: 4 * this.cornerWidth, 
                minheight: this.titlebarHeight + this.bottombarHeight, 
                ghostDrag: true
            });
            var self = this;
    ...
    redraw: function() {
        ...
        this.bodypart.setProps({ /* width: size.width,*/
            height: bodyNewH
        });
        ...
        this.heigh = size.height; //this.setProps({ top: size.top, left: size.left, width: size.width, height: size.height });
