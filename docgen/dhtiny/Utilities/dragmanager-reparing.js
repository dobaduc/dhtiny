...
$dh.dragger.addMode("RESIZE", {
    ...
    onMouseMove: function(sender, target, ev) {
        if ($dh.dragger.dragStarted && $dh.dragger.activeObj)
            return false;
    ...
    onDragStart: function(sender, target, ev) {
        ...
        if (target.dragParams.noMask != true)
            ...
            $dh.dragger.mask.style.cursor = document.body.style.cursor;
        }
    },
    ...
    onDragStop: function(sender, target, ev) {
        ...
        if ($dh.dragger.mask.style.display == "none") return;        
...
$dh.dragger.addMode("MOVE", {
    ...
    onDragging: function(sender, target, ev) {
        ...
        var mousePos = ev.pos;
        var newPos = {};
        newPos.left = mousePos.left - $dh.dragger.mouseOffset.left;
        newPos.top = mousePos.top - $dh.dragger.mouseOffset.top;
        for (var p in newPos) {
